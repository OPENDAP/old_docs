...
#	(r"~\\ref{([^}]*)}",(lambda : r" ---\1---"),dummy),
	(r"``(.*?)''", (lambda :r'"\1"'), dummy),
	(r"\\subsubsection{(.*?)}", (lambda : r"\n===\1===\n"), dummy),
	(r"\\subj{(.*?)}", (lambda : r"\n===\1===\n"), dummy),
	(r"\\subsection{(.*?)}", (lambda : r"\n==\1==\n"), dummy),
	(r"\\section{(.*?)}", (lambda : r"\n==\1==\n"), dummy),
	(r"\\chapter{(.*?)}", (lambda : r"\n=\1=\n"), dummy),
	(r"\\xlink{(.*?)}{(.*?)}", (lambda : r"[\2 \1]"), dummy),
	(r"\\xlinkn{(.*?)}{(.*?)}", (lambda : r"[\2 \1]"), dummy),
	(r"\\ind{(.*?)}", (lambda : r"\1"), dummy),
	(r"\\indc{(.*?)}", (lambda : r""), dummy),
	(r"\\_", (lambda :"_"), dummy),
...
#	(r"[^\\]?\}", None, dummy),
	...
	(r"\\lit{\.(.*?)}", (lambda : r"<code>.\1<\code>"), dummy),
	(r"\\lit{(.*?)}", (lambda : r"<code>\1<\code>"), dummy),
	(r"\\new{(.*?)}", (lambda : r"\1"), dummy),
	(r"\\class{.*?}", (lambda : r"<code>\1<code>"), dummy),
	(r"\\var{.*?}", (lambda : r"<code>\1<code>"), dummy),
	(r"\\notag", None, dummy),
# OPeNDAP specific
        (r"\\T", None, dummy),
        (r"\\opendap\\", (lambda : r"OPeNDAP"), dummy),
        (r"\\opendap", (lambda : r"OPeNDAP"), dummy),
        (r"\DataDDS", (lambda : r"DataDDS"), dummy),
        (r"\GRrADS", (lambda : r"GRrADS"), dummy),
        (r"\\Cpp\\", (lambda : r"C++"), dummy),
        (r"\\Cpp", (lambda : r"C++"), dummy),

        (r"\\begin{vcode}", (lambda : "<pre>"), start_verbatim),
        (r"\\end{vcode}", (lambda : "</pre>"), end_verbatim)
]

# in_stream  = sys.stdin;
# out_stream = sys.stdout
in_stream  = open("quick.tex", 'r')
out_stream = open("quickStart.txt", 'w')

# precompile regular expressions
tr_list3 = map(lambda x: (re.compile(x[0]), x[1], x[2]), tr_list2)

label = re.compile(r"\\label{(.*?)}")
xlink1 = re.compile(r"\\xlinkn{(.*?)%?")
xlink2 = re.compile(r"\\xlinkn{(.*?)}{(.*?)}")

for line in in_stream.readlines():
	if xlink1.search(line):
		if xlink2.search(line):
			lines0.append(line)
		else:
			if xline != None:
				xline = string.replace(xline, "\n", "")
				tline = string.join([xline, line], "")
				print "tline1: " + tline
				lines0.append(tline)
				xline = None
			else:
