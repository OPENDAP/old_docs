...
To get this program to compile on eddy use the following line:

g++ idlgif.cc -o idlgif -lidl_rpc -I../../../include -I/usr/local/rsi/idl/external/rpc -L/usr/local/rsi/idl/bin/bin.alpha -w
...
setenv LD_LIBRARY_PATH "/usr/local/rsi/idl/bin/bin.alpha"
*/
#include "idl_rpc.h"
#include<string>
#include<strings.h>
...
string dodstype;
/* These first three functions just consern themselves with converting from 
strings to integers and back again.
There really should be premade functions somewhere to do this, 
but I didn't bother looking them up.
*/
...
/* This function makes a get_dods call and puts the resulting array into the IDL 
variable specified in name.  Note that the function is
smart enough to handel 2, 3 and 4 dimensional cases.  The perl code that calls 
this program is generalized to do 2+ dimensions, and
it should be quite straight forward to generalize this code as well.  However, 
Peter has never heard of a DODS dataset with more then
...
void get_dods(string name, string url, string variable, string xlo, string xhi, string ylo, string yhi, string third, string forth) {
  ...
  if(!(forth=="")) IDL_RPCExecuteStr(idlClient,(char *)(("stat = GET_DODS('"+url+"',CE='"+variable+"."+variable+"["+forth+"]["+third+"]["+ylo+":"+yhi+"]["+xlo+":"+xhi+"]',data)").c_str()));
  else if(!(third=="")) {
    //printf(("stat = GET_DODS('"+url+"',CE='"+variable+"."+variable+"["+third+"]"+"["+ylo+":"+yhi+"]["+xlo+":"+xhi+"]',data)\n").c_str());
    IDL_RPCExecuteStr(idlClient,(char *)(("stat = GET_DODS('"+url+"',CE='"+variable+"."+variable+"["+third+"]"+"["+ylo+":"+yhi+"]["+xlo+":"+xhi+"]',data)").c_str()));
  }
  else {
    ...
    if(dodstype=="not_grid") IDL_RPCExecuteStr(idlClient,(char*)(("stat = GET_DODS('"+url+"?"+variable+"["+ylo+":"+yhi+"]["+xlo+":"+xhi+"]',data)").c_str()));
    else IDL_RPCExecuteStr(idlClient,(char*)(("stat = GET_DODS('"+url+"',CE='"+variable+"."+variable+"["+ylo+":"+yhi+"]["+xlo+":"+xhi+"]',data)").c_str()));
  }
...
/* this seems to be wrong:  printf(("cd, '"+client_dir+"'").c_str());
*/
...
*/
      ...
      IDL_RPCExecuteStr(idlClient,"cd, '/usr/local/apache/htdocs/dodsview'");
      IDL_RPCExecuteStr(idlClient,(char*)(("print, 'dods_gif, a, "+output+orientation+", "+FillValue+"'").c_str()));
      IDL_RPCExecuteStr(idlClient,(char*)((script+", a, '"+output+"', "+lonlow+", "+lonhigh+", "+latlow+", "+lathigh+", '"+orientation+"', "+FillValue + ",mask").c_str()));
