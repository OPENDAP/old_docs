...
# OPeNDAP specific
        (r"\\T", None, dummy),
        (r"\\opendap\\", (lambda : r"<nop>OPeNDAP"), dummy),
        (r"\\opendap", (lambda : r"<nop>OPeNDAP"), dummy),
        (r"\DataDDS", (lambda : r"<nop>DataDDS"), dummy),
        (r"\GRrADS", (lambda : r"<nop>GRrADS"), dummy),
...
in_stream  = open("writingClient.tex", 'r')
out_stream = open("writingClient.txt", 'w')

# precompile regular expressions
tr_list3 = map(lambda x: (re.compile(x[0]), x[1], x[2]), tr_list2)

label = re.compile(r"\\label{(.*?)}")
xlink1 = re.compile(r"\\xlink{(.*?)")
xlink2 = re.compile(r"\\xlink{(.*?)}{(.*?)}")
