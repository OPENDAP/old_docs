...
\eseChange{004.0.09, 28 August 2007, Added missing double quotes to
  grammar rules for \emph{array-dim} on
  ...
  reverse order; Added a note in Appendix A.2 regarding RFC 2616's
  ...
  (page~\pageref{CRLF-binary-fix}) a note has been added to state explicitly
  that CRLF pairs in the data portion of a data response will be
  treated as data values and not as a line terminator; Removed an
  errant \lit{<CRLF>} in the finale example of
  Section~\ref{sec-examples} (page~\pageref{ex-last}).\\
  004.0.08, 27 October 2005, Added a note that the type names are
  ...
  of the
  \emph{server}.\\
  004.0.06, 12 April 2005, Changed designation from `Proposed\ldots'
