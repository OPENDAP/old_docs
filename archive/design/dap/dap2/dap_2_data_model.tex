...
\eseChange{{\bf 004.1.1}, 8 October 2007, Updated version number to
  reflect current status.\\
  {\bf 004.0.09}, 28 August 2007, Added missing double quotes to
  grammar rules for \emph{array-dim} (now called
  \emph{projection-array-dim}), corrected an error in the
  grammar for a CE (although servers can accept an \emph{id} without a
  \emph{projection-array-dim}, the grammar did not make that explicit) and
