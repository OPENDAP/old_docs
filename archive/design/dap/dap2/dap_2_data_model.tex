...
\eseChange{{\bf 004.0.09}, 28 August 2007, Added missing double quotes to
  grammar rules for \emph{array-dim} ) (now called
  \emph{projection-array-dim}), corrected an error in the
  grammar for a CE (although servers can accept an \emph{id} without an
  projection-array-dim, the grammar did not make that explicit) and
  corrected an error in the characters REQUIRED to ba accepted for an
  \emph{id} (most servers accept more characters, but the required set
  is fairly restricted) on
  ...
  $\rightarrow$ 'dataset'; Added a note in Appendix A.2 regarding RFC 2616's
  suggestion that 'tolerant' applications ignore the CR and use only
  LF as a separator when they process a CRLF pair - we suggest not
  assuming HTTP software will follow this advice; In
  Section~\ref{sec-http-caching} (page~\pageref{sec-http-caching})
  'includ' $\rightarrow$ 'include'; In Section~\ref{sec-examples}
  (page~\pageref{sec-examples}) The value of the 'XDODS-Server' has
  been changed to more accurately reflect typical values; Added a note
  that \DAP servers MUST return the Last-Modified header in
...
and HDF5\cite{NCSA:HDF5}. They also appear under the monicker `property' in
